...
def split_into_chapters(text):
    ...
    chapters = []
    for i, (_, number, rest) in enumerate(matches):
        title_line, *paragraph_lines = rest.strip().split("\n")
        if not paragraph_lines:
            continue
        title = f"{number} {title_line.strip()}"
        paragraphs = [p.strip() for p in re.split(r"\n{1,}", "\n".join(paragraph_lines)) if len(p.strip()) > 30]
        if not paragraphs:
            continue
        chapters.append({
            "id": f"chapter_{i+1}",
